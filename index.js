...
app.use((req, res, next)=>{
    res.header("Access-Control-Allow-Origin", "*")  // watch out
    res.header(
...
app.post("/upload", upload.single("file"), (req, res)=>{
    ...
    const outputPath = `./uploads/movies/${videoId}`;
    ...
    //( proof of concept ), not to be used in production
    exec(ffmpegCommand, (error, stdout, stderr)=>{
        ...
        const videoUrl = `http://localhost:8000/uploads/movies/${videoId}/index.m3u8`;

        res.json({
            message : "Video converted to HSL format",
            videoUrl : videoUrl,
            videoId : videoId
        })
